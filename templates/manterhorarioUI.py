...
class ManterHorarioUI:
    ...
    def listar():
        ...
        if len(horarios) == 0:
            ...
            for obj in horarios:
                ...
                descricao_servico = servico.get_descricao() if servico else ""
                dic.append({
                    "ID": obj.get_id(),
    ...
    def inserir():
        ...
        cliente = st.selectbox("Informe o cliente", clientes, index=None, format_func=lambda x: x.get_nome())
        servico = st.selectbox("Informe o serviço", servicos, index=None, format_func=lambda x: x.get_descricao())
    ...
    def atualizar():
        ...
        op = st.selectbox("Atualização de Horários", horarios, format_func=lambda x: f"{x.get_id()} - {x.get_data().strftime('%d/%m/%Y %H:%M')}")
    ...
    def excluir():
        ...
        op = st.selectbox("Exclusão de Horários", horarios, format_func=lambda x: f"{x.get_id()} - {x.get_data().strftime('%d/%m/%Y %H:%M')}")
