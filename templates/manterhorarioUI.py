...
class ManterHorarioUI:
    ...
    def listar():
        ...
        for h in horarios:
            st.markdown(f"### ID: {h.get_id()}")
            st.write("Data:", h.get_data().strftime("%d/%m/%Y %H:%M"))
            st.write("Confirmado:", "Sim" if h.get_confirmado() else "Não")
            st.write("Cliente:", clientes.get(h.get_id_cliente(), "N/A"))
            st.write("Serviço:", servicos.get(h.get_id_servico(), "N/A"))
            st.write("Profissional:", profissionais.get(h.get_id_profissional(), "N/A"))
            st.markdown("---")
    ...
    def inserir():
        ...
        data = st.text_input("Data e hora (dd/mm/yyyy HH:MM)", datetime.now().strftime("%d/%m/%Y %H:%M"))
    ...
    def atualizar():
        ...
        servicos = View.servico_listar()
        profissionais = View.profissional_listar()
        op = st.selectbox("Selecione um horário", horarios, format_func=lambda h: f"{h.get_id()} - {h.get_data().strftime('%d/%m/%Y %H:%M')}")
        data = st.text_input("Nova data", op.get_data().strftime("%d/%m/%Y %H:%M"))
        confirmado = st.checkbox("Confirmado", value=op.get_confirmado())
        cliente = st.selectbox("Cliente", clientes, index=next((i for i, c in enumerate(clientes) if c.get_id() == op.get_id_cliente()), 0), format_func=lambda c: c.get_nome())
        servico = st.selectbox("Serviço", servicos, index=next((i for i, s in enumerate(servicos) if s.get_id() == op.get_id_servico()), 0), format_func=lambda s: s.get_descricao())
        profissional = st.selectbox("Profissional", profissionais, index=next((i for i, p in enumerate(profissionais) if p.get_id() == op.get_id_profissional()), 0), format_func=lambda p: p.get_nome())

        if st.button("Atualizar"):
            try:
                data_fmt = datetime.strptime(data, "%d/%m/%Y %H:%M")
                View.horario_atualizar(op.get_id(), data_fmt, confirmado, cliente.get_id(), servico.get_id(), profissional.get_id())
